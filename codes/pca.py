...
class PCA_Analysis:
    ...
    def loadings(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        loadings = pd.DataFrame(
            pca.components_.T, columns=["PC1", "PC2", "PC3"], index=df.columns
        )

        return loadings

    def top_components(self, loadings: pd.DataFrame, n: int) -> list:
        """
        ...
        struct_attributes = [
            attr
            for attr in top_attributes_pc1.index
            if attr in top_attributes_pc2.index
        ]
    ...
    def explained_variance(self, df) -> pd.DataFrame:
        ...
        pca_df = pd.DataFrame(["", "PC1", "PC2", "PC3"], columns=["PC"])
    ...
    def list_shared_attr (self, top_components_dict: dict) -> list:
        ...
        max_length = max([len(top_components_dict[key]) for key in top_components_dict.keys()])
        ...
        top_components_df = top_components_df.reindex(sorted(top_components_df.columns), axis=1)  
